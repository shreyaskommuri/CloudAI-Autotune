...
class CloudAIRunner:
    ...
    def __init__(
        ...
        self.hook_dir = Path(hook_dir) if hook_dir is not None else None
    ...
    def _build_command(self, config_path: Path, report_path: Path) -> list[str]:
        if self.system_config is not None:
            ...
            if self.hook_dir is not None:
                cmd.extend(["--hook-dir", str(self.hook_dir)])
            return cmd
